...
<Meta title="Documentation/Getting Started/Formly" />
...
⚠️ IMPORTANT: Formly 7.x is not yet compatible with `wcs-formly` which relies on Formly 6.x. Please install version `6.x.x` of Formly for now.

```
ng add @ngx-formly/schematics@6
// or
npm install @angular/forms @ngx-formly/core@6 --save
```

The getting started below rely on Formly 6.x version. Double check with the official Formly documentation during your installation process.

<wcs-tabs aria-label="Formly installation steps">
  <wcs-tab header="Angular Standalone" item-key="formlyStandaloneTab">
    2. **Import providers in `importProvidersFrom` :**
    ...
    3. **Import `FormlyModule` in each component using Formly form :**

    ```js
    @Component({
      selector: 'component-name-selector',
      standalone: true,
      imports: [
        WcsFormlyModule,
        ...
    ...
    4. **Add a `formly-form` inside your template form**
    ...
    5. **Configure your form in the component definition**
    ```ts
  ...
  <wcs-tab header="Angular Module" item-key="formlyModuleTab">
...
<wcs-tabs aria-label="Formly installation steps">
  <wcs-tab header="Angular Standalone" item-key="formlyStandaloneTab">
    6. **Import `WcsFormlyModule` in each component using Formly form :**
  ...
  <wcs-tab header="Angular Module" item-key="formlyModuleTab">
    6. **Import the `WcsFormlyModule` in your module declaration :** 
...
<form>
  <formly-form style={{display: 'flex', flexDirection: 'column', gap: 'var(--wcs-semantic-spacing-large)', marginBottom: '2em'}}>
    <formly-field>
...
```ts
// app.module.ts or in importProvidersFrom
FormlyModule.forRoot({
      types: [
        {
...
**See more examples :**  
- WCS Angular example project : [example/angular/src/app/example/formly-example](https://gitlab.com/SNCF/wcs/-/tree/master/example/angular/src/app/example/formly-example)
